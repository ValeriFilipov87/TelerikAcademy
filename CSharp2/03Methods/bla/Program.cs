...
namespace _08_Number_As_Array
{
    class NumberAsArrays
    {
        ...
        public static int[] GetSum(int[] NumberOne, int[] NumberTwo)
        {
            ...
            for (int curDigit = 0; curDigit < Math.Min(NumberOne.Length, NumberTwo.Length); curDigit++)
            {
                curSum = 0;
                curSum = NumberOne[curDigit] + NumberTwo[curDigit] + CarriedOver;
                CarriedOver = 0;
                if (curSum > 9)
                {
                    curSum %= 10;
