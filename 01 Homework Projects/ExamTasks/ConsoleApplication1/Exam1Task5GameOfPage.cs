...
class Program
{
    static void Main()
    {
        ...
        string commandWhat = "what is";
        string commandBuy = "buy";

        // counter for sold cookies
        int cookieSold = new int();
        double price = 1.79;
        ...
        while (true)
        {
            ...
            // check for a crumb
            bool isCrumb = (middleRow & center) != 0;

            // check for broken cookie
            bool isBrokenCookie =
                isCrumb && (
                //upper
                ((upperRow & (1 << (col - 1))) != 0) ||
            ...
            if (command == commandBuy)
            {
                if (isWholeCookie)
                {
                    ...
                    // clean the cookie after buying it
                    switch (row)
                    {
        ...
        Console.WriteLine("{0:F2}", cookieSold * price);
    }
}
